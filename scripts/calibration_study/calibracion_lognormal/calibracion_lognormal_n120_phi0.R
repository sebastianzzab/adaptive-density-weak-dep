###########################################################
# Calibración de Lognormal - Método GL
###########################################################
...
Vh_vectorizado <- function(H_vec, n, gsup, gamma, phi) {
  ...
  K1 <- 1
  K2 <- sqrt(1 / (2 * sqrt(pi)))
  return(sqrt(2 * gamma * gsup) * K2 * (K1 + 1) * (((log(n))^(-1/2)) / sqrt(n * H_vec)))
}
...
directorio_salida <- setwd("~/Desktop/tesis_sebastian/adaptive-density-weak-dep/scripts/calibration_study/calibracion_lognormal")
png(
