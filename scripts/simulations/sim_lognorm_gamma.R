###############################################################################
# Script: Estudio de Simulación Monte Carlo - Grupo 1 (Lognormal y Gamma)
...
# =====================================================================
# 1. CONFIGURACIÓN GLOBAL Y SEMILLAS
# =====================================================================
# Instrucción: Para cambiar el tamaño de muestra, comenta/descomenta el bloque deseado.

# --- CORRIDA 1: n = 60 ---
# CONFIG_n <- 60
# CONFIG_semilla_maestra <- 1060

# --- CORRIDA 2: n = 120 ---
# CONFIG_n <- 120
# CONFIG_semilla_maestra <- 1120

# --- CORRIDA 3: n = 250 ---
CONFIG_n <- 250
CONFIG_semilla_maestra <- 1250

# Parámetros fijos del experimento
CONFIG_B <- 1000
CONFIG_niveles_phi <- c(0.3, 0.5, 0.9)
CONFIG_distribuciones <- c("lognormal", "gamma")
CONFIG_ruta_salida <- "./resultados_simulacion/Grupo1_Asimetricas"

if(!dir.exists(CONFIG_ruta_salida)) dir.create(CONFIG_ruta_salida, recursive = TRUE)

# =====================================================================
# 2. TABLA INTERNA DE GAMMAS ÓPTIMOS
# =====================================================================
# SUSTITUYE LOS VALORES 0.05 POR LOS NÚMEROS EXACTOS DE TU TABLA EXCEL
tabla_gammas <- data.frame(
  n = c(rep(60, 3), rep(120, 3), rep(250, 3)),
  phi = rep(c(0.3, 0.5, 0.9), times = 3),
  gamma_optimo = c(
    0.15625, 0.02, 0.18125, # Valores para n = 60  (phi = 0, 0.5, 0.9)
    0.13125, 0.015, 0.0775, # Valores para n = 120 (phi = 0, 0.5, 0.9)
    0.3875,	0.0325, 0.0675  # Valores para n = 250 (phi = 0, 0.5, 0.9)
  )
)
