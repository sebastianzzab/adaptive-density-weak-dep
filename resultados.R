...
generar_muestra <- function(n, phi, tipo, mw_idx = 1, a = -2, b = 2, me, de) {
  ...
  if (tipo == "truncnorm") {
    ...
    X <- qnorm(U, mean = me, sd = de)  # Pasar media y desviación
    gsup <- 1 / (sqrt(2 * pi) * de)
    y_teo_func <- function(x) dnorm(x, mean = me, sd = de) # Convertir a función anónima
...
analizar_y_graficar_replicacion <- function(n, phi, tipo_densidad, mw_idx = 1, 
                                            ...
                                            generar_plot = TRUE, me = 0, de = 1) {
  
  # 1. Generación de muestra universal (AQUÍ SE PASAN me Y de)
  info_muestra <- generar_muestra(n = n, phi = phi, tipo = tipo_densidad, 
                                  mw_idx = mw_idx, a = a, b = b, me = me, de = de)
  X <- info_muestra$X
...
# resultados_X11 <- analizar_y_graficar_replicacion(n = 250, phi = 0.9, tipo_densidad = "normal-est", gamma_gl = 0.05, )
# resultados_X11

resultados_X <- analizar_y_graficar_replicacion(n = 60, phi = 0.9, tipo_densidad = "normal", gamma_gl = 0.05, me = 0, de = 3)
resultados_X
